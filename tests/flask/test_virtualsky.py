...
from indi_allsky.lens_solver.projection import predictAltAz, projectToPixels, precessCatalog
...
def test_browser_and_solver_agree_across_timestamps(altitude, heading, precession):
    ...
    payload = dict(cases=cases, altitude=altitude, heading=heading, precession=precession)
    run_node('-e', '''
...
for (const asset of ['virtualsky.js', 'virtualsky.min.js']) {
  for (const c of input.cases) {
    const sky = makeSky({fisheye_altitude: input.altitude, fisheye_azimuth: input.heading,
      az: 20, latitude: 48.51, longitude: 6.5, precession: input.precession,
    ...
    if (Math.hypot(c.x-500, c.y-500) > 500 && input.altitude !== 90) {
      ...
      if (Math.hypot(p.x-500, p.y-500) < 500) {
