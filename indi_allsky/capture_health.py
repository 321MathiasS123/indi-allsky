...
def capture_health_status(
    ...
    health = {
        ...
        'last_success_timestamp': latest_success_timestamp,
        'capture_expected': bool(capture_expected),
...
def local_capture_health(config, camera, night):
    """Shared Latest-page and automation evidence, including successful image age."""
    ...
    from .flask.models import IndiAllSkyDbImageTable, IndiAllSkyDbCameraTable

    state_db = miscDb(config)
    now_timestamp = time.time()
    stale_seconds = capture_stale_seconds(config, night)
    ...
    if camera.capture_pause:
        capture_expected = False
    elif not night and not camera.daytime_capture:
        capture_expected = False


    try:
        active_camera_id = int(state_db.getState('DB_CAMERA_ID'))
    except (NoResultFound, TypeError, ValueError):
        active_camera_id = None

    if active_camera_id is not None and active_camera_id != camera.id:
        return capture_health_status(
            now_timestamp,
            None,
            None,
    ...
    try:
        capture_status = int(state_db.getState('STATUS'))
    except (NoResultFound, TypeError, ValueError):
        capture_status = None
    ...
    image_dir = Path(config['IMAGE_FOLDER']).absolute()
    latest_image_p = image_dir.joinpath(
        'latest.{0:s}'.format(
            config.get('IMAGE_FILE_TYPE', 'jpg'),
        )
    )
    ...
    try:
        failure = parse_capture_error(
            state_db.getState(CAPTURE_PIPELINE_ERROR_STATE),
        )
